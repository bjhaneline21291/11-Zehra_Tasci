...
from mask_datasets.mask import ImageDataset
...
args = args(dict(backbone="resnet50", dilation=True, hidden_dim=256, position_embedding='learned', masks=False,
                 ...
                 eos_coef=0.1, giou_loss_coef=2,lr_backbone=1e-5, lr=1e-4, weight_decay=1e-4, num_classes=2, num_queries=20))
...
class DETRModel(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        # Create a transform
        transform = T.Compose([T.NormalizeBoxes(), T.Resize([400, 400]), T.ToTensor()])

        # Create the datasets
        dataset = ConcatDataset([ImageDataset(images=images_1, annotation_folder="dataset/Face Mask/527030_966454_bundle_archive/labels",
        ...
        return DataLoader(dataset=dataset, batch_size=4, shuffle=True, collate_fn=ImageDataset.collate_fn, num_workers=4)
...
#%% Evaluation
# Load the checkpoint
model = DETRModel.load_from_checkpoint(checkpoint_path="lightning_logs/version_12/checkpoints/epoch=36.ckpt")

for image in glob(f"dataset/Face Mask/eval/1.jpg"):
    # Load and transform the image
    x = Image.open(fp=image, mode='r').convert('RGB')
    x = F.resize(img=x, size=[400, 400])
    ...
    label_map = {"mask": 0, "no mask": 1, "background":2}
